...
export class GraczeComponent implements OnInit {
    ...
    forkJoin({ gracze: this.api.getGracze(), wyniki: this.api.getWyniki() }).subscribe({
      next: ({ gracze, wyniki }) => {
        this.gracze = gracze.map(g => {
          ...
          return { ...g, sumaRekordow: Math.round(suma * 10) / 10, liczbaSkoczni: maxPerSkocznia.size };
        });
  ...
  openAdd(): void { this.form = { login_gracza: '' }; this.editMode = false; this.editId = null; this.showModal = true; }

  openEdit(g: Gracz): void { this.form = { ...g }; this.editMode = true; this.editId = g.id!; this.showModal = true; }
  ...
  save(): void {
    ...
    obs.subscribe({
      next: () => { this.success = this.editMode ? 'Zaktualizowano!' : 'Dodano gracza!'; this.showModal = false; this.load(); },
